...
- climbStairsInplace: bottom-up DP using O(1) extra space
  (O(n) time, O(1) space)
...
The problem is equivalent to computing the (n+1)th Fibonacci number when
starting from 0 -> 1 -> 1 -> 2 -> ...
"""
...
def climbStairsRecursion(n):
    ...
    Notes:
        - Uses recurrence ways(n) = ways(n-1) + ways(n-2).
        - Time complexity: O(2^n) in the worst case (exponential).
        - Space complexity: O(n) due to recursion stack.
    """
    # Base cases: n == 0 or n == 1 -> exactly 1 way
    if n <= 1:
        return 1
...
def climbStairsInplace(n):
    ...
    Instead of storing all dp values, track only the last two counts
    (prev2 for i-2, prev1 for i-1) and compute the current number of ways.
    ...
    # Iteratively compute ways(i) for i from 2 to n. Variable name is
    # unused inside loop, so use an underscore to avoid B007.
    for _ in range(2, n + 1):
        ways = prev1 + prev2  # current ways based on previous two
        # shift window: prev2 becomes previous (i-1), prev1 becomes current
        prev2 = prev1
        prev1 = ways
